...
async function handleSend(text) {
  ...
  try {
    ...
    AuraVoice.speak(data.reply.content);
    // Roughly estimate speaking duration to reset the state indicator;
    // browsers don't expose a reliable "done speaking" callback everywhere.
    const estimatedMs = Math.min(12000, data.reply.content.length * 55);
    setTimeout(() => setAuraState("idle"), estimatedMs);
